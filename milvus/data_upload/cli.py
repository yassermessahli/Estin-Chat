...
def main():
    ...
    parser.add_argument("--get-records-from-jsonl", nargs=2, metavar=("INPUT_FOLDER", "OUTPUT_FOLDER"),
                       help="Extract records from JSONL files and save to JSON")
    parser.add_argument("--push-bulk-to-minio", metavar="INPUT_FOLDER",
                       help="Push bulk data to MinIO")
    parser.add_argument("--populate-milvus", nargs="*", metavar="BATCH_FILE",
                       help="Import batch files from MinIO to Milvus (specify files or leave empty for all)")
    parser.add_argument("--get-import-status", metavar="JOB_ID",
                       help="Get import status for job ID")
