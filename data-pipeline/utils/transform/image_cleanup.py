...
from .prompts import PromptTemplate, IMAGE_CLEANUP_PROMPT, IMAGE_CLEANUP_SYSTEM_PROMPT, OUTPUT_SCHEMA


class ImageCleanup:
    def __init__(self, image_data: dict = None, model: Model = None, output_schema: str = OUTPUT_SCHEMA):
        self.img = image_data["base64"] if image_data is not None else ""
        self.ext = image_data["ext"] if image_data is not None else ""
        self.model = model

        self.instruction = PromptTemplate(template=IMAGE_CLEANUP_PROMPT, image_extension=self.ext).prompt
