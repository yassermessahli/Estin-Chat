...
def classify_query_filters(query: str) -> dict:
    ...
    try:
        response = ollama.chat(model="sam860/qwen3:4b-Q3_K_XL", messages=[
...
# -------------------------------
# Step 3: Send final context to LLM (STREAMING VERSION)
# -------------------------------
def generate_answer_streaming(query: str, context_chunks: list):
    """Use LLM (Qwen) to generate the final answer with streaming."""
    ...
    with open("context_output.txt", "w", encoding="utf-8") as f:
        f.write("🧠 CONTEXT USED FOR ANSWER GENERATION:\n")
        f.write("-" * 60 + "\n")
        
        for i, chunk in enumerate(context_chunks[:5]):
            if isinstance(chunk, dict):
                source_line = f"📄 Source {i+1}: {chunk['title']}"
                score_line = f"🎯 Score: {chunk['score']:.4f}"
                content_line = f"📝 Content: {chunk['content']}"
                separator = "-" * 60
    ...
    try:
        stream = ollama.chat(
            model="sam860/qwen3:4b-Q3_K_XL", 
            messages=[
                {"role": "system", "content": "/no_think You are a helpful academic assistant. Answer clearly and accurately based only on the provided context."},
...
def print_streaming_response(query: str, context_chunks: list):
    """Print the streaming response word by word with a typewriter effect."""
    print(f"\n🤖 Assistant: ", end="", flush=True)
    
    full_response = ""
    try:
        for chunk in generate_answer_streaming(query, context_chunks):
            print(chunk, end="", flush=True)
            full_response += chunk
            # Add a small delay for typewriter effect (optional - remove if too slow)
...
# -------------------------------
# Step 4: Full RAG Chain (Updated for Streaming)
# -------------------------------
def rag_respond_streaming(query: str):
    ...
    end1 = time.time()
    print(f"🧠 step 1 Time: {end1 - start1:.2f} seconds")
    ...
    end2 = time.time()
    print(f"🧠 step 2 Time: {end2 - start2:.2f} seconds")
    
    if not chunks:
        print("\n🤖 Assistant: I couldn't find any relevant documents to answer your question. This might be due to collection configuration issues or the documents may not be indexed properly.")
        return

    # Step 4: Generate streaming answer
    print(f"\nStep 3: Generating streaming answer using {len(chunks)} document(s)...")
    start3 = time.time()
    
    # Print streaming response
    response = print_streaming_response(query, chunks)
    
    end3 = time.time()
    print(f"\n🧠 Step 3 Time: {end3 - start3:.2f} seconds")
...
# -------------------------------
# CLI (Updated for Streaming)
# -------------------------------
if __name__ == "__main__":
    print("RAG Chatbot (Milvus + Qwen) - STREAMING VERSION 🚀")
    print("=" * 70)
    ...
    print("🌟 Now with real-time streaming responses! 🌟\n")
    ...
    while True:
        ...
        if query.lower() in ("exit", "quit"):
            ...
            print("- The system now streams responses in real-time!")
            print("- Similarity scores are shown for retrieved documents")
            continue
        ...
        try:
            ...
            rag_respond_streaming(query)
            end_final = time.time()
            print(f"🧠 Total Time: {end_final - start_final:.2f} seconds")
            print("=" * 70)
        except Exception as e:
            print(f"❌ Error: {e}")
            print("=" * 70)